...
macro_rules! trait_impl {
    ($context:ident, $context_struct:path) => {
        impl ThreadSyscalls for $context_struct {
            fn k_wakeup(thread: ThreadId) {
                unsafe { std::sys::zephyr::syscalls::$context::k_wakeup(thread.tid()) }
            }

            fn k_current_get() -> crate::thread::ThreadId {
                ThreadId(unsafe {
                    NonNull::new_unchecked(std::sys::zephyr::syscalls::$context::k_current_get())
                })
            }

            fn k_object_access_grant<K: KObj>(kobj: &K, thread: ThreadId) {
                if !std::sys::zephyr::raw::RUST_CONFIG_USERSPACE {
                    // Avoid unnecessary call to stub function
                    return;
                }
                unsafe {
                    std::sys::zephyr::syscalls::$context::k_object_access_grant(
                        kobj.as_void_ptr(),
                        thread.tid(),
                    );
                }
