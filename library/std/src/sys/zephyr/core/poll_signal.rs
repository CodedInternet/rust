...
super::make_static_wrapper!(k_poll_signal, zephyr_sys::raw::k_poll_signal);
...
macro_rules! trait_impl {
    ($context:ident, $context_struct:path) => {
        impl KPollSignalSyscalls for $context_struct {
            unsafe fn k_poll_signal_init(signal: &KPollSignal) {
                std::sys::zephyr::syscalls::$context::k_poll_signal_init(signal as *const _ as *mut _)
            }

            fn k_poll_signal_reset(signal: &KPollSignal) {
                unsafe {
                    std::sys::zephyr::syscalls::$context::k_poll_signal_reset(
            ...
            fn k_poll_signal_check(
                ...
                unsafe {
                    std::sys::zephyr::syscalls::$context::k_poll_signal_check(
                        signal as *const _ as *mut _,
                        signaled,
                        result,
                    )
            ...
            fn k_poll_signal_raise(signal: &KPollSignal, result: c_int) -> c_int {
                unsafe {
                    std::sys::zephyr::syscalls::$context::k_poll_signal_raise(
                        signal as *const _ as *mut _,
                        result,
                    )
                }
...
trait_impl!(kernel, super::context::Kernel);
trait_impl!(user, super::context::User);
trait_impl!(any, super::context::Any);
